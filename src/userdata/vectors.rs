...
impl LuaUserData for Vector2 {
    ...
    fn add_methods<M: LuaUserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method(
            "Lerp",
            |_, this, (other, alpha)| Ok(this.lerp(other, alpha)),
        );
...
impl LuaSingleton for Vector2 {
    fn register_singleton(lua: &Lua) -> LuaResult<()> {
        ...
        table.raw_set(
            "new",
            lua.create_function(|_, (x, y): (f64, f64)| Ok(Vector2 { x, y }))?,
        )?;
...
impl Vector3 {
    ...
    pub const X_AXIS: Vector3 = Vector3 {
        x: 1f64,
        y: 0f64,
        z: 0f64,
    };
    pub const Y_AXIS: Vector3 = Vector3 {
        x: 0f64,
        y: 1f64,
        z: 0f64,
    };
    pub const Z_AXIS: Vector3 = Vector3 {
        x: 0f64,
        y: 0f64,
        z: 1f64,
    };
...
impl LuaUserData for Vector3 {
    ...
    fn add_methods<M: LuaUserDataMethods<Self>>(methods: &mut M) {
        ...
        methods.add_method(
            "Angle",
            |_, this, (other, axis): (Vector3, Option<Vector3>)| Ok(this.get_angle(other, axis)),
        );
        methods.add_method("Dot", |_, this, other| Ok(this.dot(other)));
        methods.add_method(
            "Lerp",
...
impl LuaSingleton for Vector3 {
    fn register_singleton(lua: &Lua) -> LuaResult<()> {
        ...
        table.raw_set(
            "new",
            lua.create_function(|_, (x, y, z): (f64, f64, f64)| Ok(Vector3 { x, y, z }))?,
        )?;
        table.raw_set(
            "FromNormalId",
            lua.create_function(|_, v: NormalId| Ok(Vector3::from(v)))?,
        )?;
        table.raw_set(
            "FromAxis",
            lua.create_function(|_, v: Axis| Ok(Vector3::from(v)))?,
        )?;
