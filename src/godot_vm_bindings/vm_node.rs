...
use godot::{
    classes::{node::InternalMode, Engine, FileAccess, RichTextLabel},
    global::Error,
    prelude::*,
};
...
use crate::{
    core::{
        borrowck_ignore_mut, get_state, FastFlag, FastFlagValue, GlobalTaskScheduler,
        ParallelDispatch::Synchronized, RblxVM, RwLock, ThreadIdentity, ThreadIdentityType,
    },
    instance::escape_bbcode_and_format,
};
...
impl INode for RblxVMNode {
    fn init(owner: Base<Node>) -> Self {
        ...
        for (flag, name) in FastFlag::type_info()
            .as_enum()
            .unwrap()
            .variant_names()
            .iter()
            .enumerate()
        {
            let _ = dict.insert(
                *name,
                match FastFlag::get_default(unsafe { transmute(flag as u16) }) {
    ...
    fn ready(&mut self) {
        ...
        let variants: HashMap<&str, usize> = FastFlag::type_info()
            .as_enum()
            .unwrap()
            .variant_names()
            .iter()
        ...
        for (k, v) in self.startup_flags.iter_shared() {
            if let Err(e) = (|| {
                let k = k
                    .try_to::<GString>()
                    .map_err(|_| "key in dictionary is not string")?;
                let fastflag_id = *variants
                    .get(k.to_string().as_str())
                    .ok_or("key is not FastFlag name")? as u16;
                match v.get_type() {
                    VariantType::BOOL => flags_table.push((
                        unsafe { transmute(fastflag_id) },
                        FastFlagValue::Bool(v.to::<bool>()),
                    )),
                    VariantType::INT => flags_table.push((
                        unsafe { transmute(fastflag_id) },
                        FastFlagValue::Int(v.to::<i64>()),
                    )),
                    VariantType::FLOAT => flags_table.push((
                        unsafe { transmute(fastflag_id) },
                        FastFlagValue::Float(v.to::<f64>()),
                    )),
                    VariantType::STRING => flags_table.push((
                        unsafe { transmute(fastflag_id) },
                        FastFlagValue::String(v.to::<GString>().to_string()),
                    )),
...
impl RblxVMNode {
    fn post_init(&mut self) {
        ...
        self.base_mut()
            .add_child_ex(&loaded_scene)
            .internal(InternalMode::FRONT)
            .done();

        let mut log_window = self.base_mut().get_node_as::<RichTextLabel>(
            "ConsoleInterface/ConsoleInterface/PanelContainer/RichTextLabel",
        );
...
impl RblxVMNode {
        ...
        (|| {
            if let Some(vm) = self.vm.as_ref() {
                let mut write = vm
                    .write()
                    .inspect_err(|_| {
                        godot_error!("RblxVMNode: failed to acquire write lock on RblxVM")
                    })
                    .map_err(|_| Error::ERR_CANT_ACQUIRE_RESOURCE)?;
                let fastflag_id = *variants
                    .get(flag.to_string().as_str())
                    .ok_or(Error::ERR_INVALID_PARAMETER)
                    .inspect_err(|_| godot_error!("RblxVMNode: invalid flag name provided"))?
                    as u16;
                ...
                let func = lua
                    .create_function_mut(move |lua, ()| {
                        match &fastflag_value {
                            FastFlagValue::Bool(v) => {
                                get_state(lua).flags().set_bool(fastflag_id, *v)
                            }
                            FastFlagValue::Int(v) => {
                                get_state(lua).flags().set_int(fastflag_id, *v)
                            }
                            FastFlagValue::Float(v) => {
                                get_state(lua).flags().set_float(fastflag_id, *v)
                            }
                            FastFlagValue::String(v) => {
                                get_state(lua).flags().set_string(fastflag_id, v.clone())
                            }
                        }
                        Ok(())
                    })
                    .unwrap();
                unsafe { vm.access().as_mut().unwrap_unchecked() }
                    .get_main_state()
        ...
        (|| {
            if let Some(vm) = self.vm.as_ref() {
                let read = vm
                    .read()
                    .inspect_err(|_| {
                        godot_error!("RblxVMNode: failed to acquire read lock on RblxVM")
                    })
                    .map_err(|_| Variant::nil())?;
                let fastflag_id = *variants
                    .get(flag.to_string().as_str())
                    .ok_or_else(|| Variant::nil())
                    .inspect_err(|_| godot_error!("RblxVMNode: invalid flag name provided"))?
                    as u16;
                let fastflag_id = unsafe { transmute(fastflag_id) };
    ...
    fn push_code(&mut self, chunk: GString) -> Error {
        self.vm.as_mut().map(|vm| {
            ...
            let func = state
                .compile_jit("<godot>", chunk.to_string().as_str(), env)
                .unwrap();
            let lua = unsafe { (&raw const *state.get_lua()).as_ref().unwrap_unchecked() };
            let thr = unsafe { borrowck_ignore_mut(state) }
                .get_task_scheduler_mut()
                .defer_func(lua, func, (), Synchronized)
                .inspect_err(|_| godot_error!("RblxVMNode: failed to defer on task scheduler"))
                .unwrap();
            state.set_thread_identity(
                thr,
                ThreadIdentity {
