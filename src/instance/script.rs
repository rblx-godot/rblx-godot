...
impl BaseScriptComponent {
    fn compile_luau(source: &String, lua: &Lua, state: &mut LuauState, script: &ManagedInstance) -> LuaResult<LuaFunction> {
        ...
        let func: LuaFunction;
        {
            ...
            if debug {
                f = state.compile_debug(
                    format!("<script at {}>", script.get_full_name()?).as_str(), source.as_str(), env
                );
        ...
        match key.as_str() {
            "Source" => {
                ...
                Some(InstanceComponent::emit_property_changed(
                    &ptr.get_instance_component(), lua, "Source", &lua_getter!(clone, lua, self.source).unwrap()))
            },
            _ => None
        }
    }

    fn clone(self: &RwLockReadGuard<'_, Self>, _lua: &Lua, new_ptr: &WeakManagedInstance) -> LuaResult<Self> {
...
impl IObject for ModuleScript {
    fn lua_get(&self, lua: &Lua, name: String) -> LuaResult<LuaValue> {
        self.module_script.read().unwrap().lua_get(self, lua, &name)
            .unwrap_or_else(|| self.instance.read().unwrap().lua_get(lua, &name))
    }

    fn get_class_name(&self) -> &'static str { "ModuleScript" }
...
impl IInstance for ModuleScript {
    ...
    fn lua_set(&self, lua: &Lua, name: String, val: LuaValue) -> LuaResult<()> {
        self.module_script.write().unwrap().lua_set(self, lua, &name, &val)
            .unwrap_or_else(|| self.instance.write().unwrap().lua_set(lua, &name, val))
    }
    ...
    fn clone_instance(&self, lua: &Lua) -> LuaResult<ManagedInstance> {
        Ok(Irc::new_cyclic_fallable::<_, LuaError>(|x| {
            ...
            let script = ModuleScript {
                instance: RwLock::new_with_flag_auto(self.get_instance_component().clone(lua, &i)?),
                module_script: RwLock::new_with_flag_auto(self.get_module_script_component().clone(lua, &i)?),
            };
            Ok(script)
        })?.cast_from_sized().unwrap())
    }
...
impl dyn IModuleScript {
    ...
    pub fn require(&self, lua: &Lua, actor: &Option<WeakManagedActor>) -> LuaResult<LuaValue> {
        ...
        let func = BaseScriptComponent::compile_luau(&component.source, lua, get_state(lua), &component._self_ptr.upgrade().unwrap())?;
        ...
        component.already_imported.insert(actor.clone(), res.clone());
        Ok(res)
    }
...
impl ModuleScript {
    pub fn new() -> ManagedInstance {
        Irc::new_cyclic(|x|
            ModuleScript {
                instance: RwLock::new_with_flag_auto(InstanceComponent::new(x.cast_to_instance(), "ModuleScript")),
                module_script: RwLock::new_with_flag_auto(ModuleScriptComponent::new(x.cast_to_instance(), "ModuleScript"))
            }
        ).cast_from_sized().unwrap()
    }
}